...
pub async fn transmition_delay(){
    ...
    tokio::time::sleep(Duration::from_nanos((thread_rng().sample(normal) * 1000.0) as u64)).await;
}

pub async fn simulate_busy(){
    let mut range = thread_rng();
    let normal = rand_distr::Normal::new(1.0, 1.0).unwrap();
    tokio::time::sleep(Duration::from_millis((range.sample(normal) * 1000.0) as u64)).await;
}
...
mod tests {
    ...
    async fn busy_as_expected(){
        ...
        let mut range = thread_rng();
        let normal = rand_distr::Normal::new(1.0, 0.1).unwrap();
        println!("{:?}", Duration::from_millis((range.sample(normal) * 1000.0) as u64));
    }
}