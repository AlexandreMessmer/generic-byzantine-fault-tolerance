...
impl PeerSystem {
    ...
    pub async fn setup(peers: usize) -> Self{
        ...
        let peer_runners: Vec<PeerRunner> = ids.zip(keys)
            .zip(senders)
            .zip(receivers)
            .zip(outlets)
            .map(|((((id, key), sender), receiver), runner_outlet)| {
                let keys_table = keys_table.clone();
                let peer = Peer {id, key, sender, receiver};
...
mod tests{
    ...
    use crate::system::{self, Message, peer_system::PeerSystem};
    ...
    async fn double_message(){
        use tokio::join;
        let system: PeerSystem = PeerSystem::setup(3).await.into();
        let t1 = system.send_command(Command::Send(2, Message::Plaintext(String::from("Hello"))), 0);
        let t2 = system.send_command(Command::Send(0, Message::Plaintext(String::from("Good bye !"))), 1);
        join!(t1, t2);
    ...
    async fn double_messages_2(){
        let system: PeerSystem = PeerSystem::setup(3).await.into();
        let t1 = system.send_command(Command::Send(2, Message::Plaintext(String::from("Hello"))), 0);
        let t2 = system.send_command(Command::Send(2, Message::Plaintext(String::from("Good bye !"))), 1);
        tokio::join!(t1, t2);
        tokio::time::sleep(Duration::from_secs(5)).await;
    ...
    async fn message_to_itself(){
        ...
        system.send_command(Command::Send(2, Message::Plaintext(String::from("Hello myself"))), 0).await;
